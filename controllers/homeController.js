...
module.exports = {
  addPlateform: async (payloadData, path) => {
    ...
    let addPlateform = await Service.homeService.saveData(objToSave);
    if (addPlateform) {
  ...
  saveData: async (payloadData) => {
    ...
    let payload = await commonHelper.verifyJoiSchema(payloadData, schema);
    let objToSave = {};
    if (_.has(payload, "userName") && payload.userName != "")
      objToSave.name = payload.userName;
    if (_.has(payload, "email") && payload.email != "")
      objToSave.email = payload.email;
    if (_.has(payload, "platformName") && payload.platformName != "")
      objToSave.platform = payload.platformName;
    if (_.has(payload, "category") && payload.category != "")
      objToSave.category = payload.category;
    if (_.has(payload, "loginType") && payload.loginType != "")
      objToSave.loginMethod = payload.loginType;
    if (_.has(payload, "profile") && payload.profile != "")
      objToSave.profile = payload.profile;
    if (_.has(payload, "mvp") && payload.mvp != "")
      objToSave.mvpFeatures = payload.mvp;
    if (
      _.has(payload, "additionalFeatures") &&
      payload.additionalFeatures != ""
    )
      objToSave.additionalFeatures = payload.additionalFeatures;
    if (_.has(payload, "applicationType") && payload.applicationType != "")
      objToSave.applicationType = payload.applicationType;
    if (_.has(payload, "totalHours") && payload.totalHours != "")
      objToSave.totalHours = payload.totalHours;

    let saveUser = await Service.homeService.saveUserData(objToSave);
    console.log("objToSave controller:  ", objToSave);
    if (saveUser) {
      var variableDetails = {
        user_applicationType:payload.applicationType,
        user_plateform: payload.platformName,
        user_category:payload.category,
        user_loginType:payload.loginType,
        user_profile:payload.profile,
        user_mvpFeatures:payload.mvp,
  ...
  addNewCategoryByUser: async (payloadData) => {
    ...
    if (_.has(payload, "category") && payload.category != "")
      objToSave.category = payload.category;
    if (_.has(payload, "platform") && payload.platform != "")
      objToSave.platform = payload.platform;
    ...
    if (saveUser) {
      var variableDetails = {
        ...
        user_category: payload.category,
        user_plateform:payload.platform,
        ip: config.APP_URLS.API_URL,
        baseUrl: config.APP_URLS.API_URL,
        s3Url: config.AWS.S3.s3Url,
      };
  ...
  getPlateform: async (payloadData) => {
    let profile = await Service.homeService.getplateformAll(payloadData);
    console.log(
  ...
  getReadyMadeCategories: async () => {
    let data = Service.homeService.getCategories({
      applicationTypeId: "0e0bb306-a7aa-4df9-9740-f03b53cb4e01",
    });
    if (data) {
      return data;
    } else {
  ...
  getCustomCategories: async () => {
    let data = Service.homeService.getCategories({
      applicationTypeId: "0e0bb306-a7aa-4df9-9740-f03b53cb4e02",
    });
    if (data) {
      return data;
    } else {
